...
$(document).ready(function(){
    ...
    var sucessEmail=$('#sucessEmail');
    //Esconde o elemento com id errolog
    $('#formRecuperaLogin').submit(function(){ 	//Ao submeter formulário
        ...
        $.ajax({			//Função AJAX
            url:"recuperar_login.php",			//Arquivo php
            ...
            success: function (result){
                ...
                $(document).ajaxStop(function(){
                    if(result==1){
                        sucessEmail.show();
                        window.location.href="formulario_login_professor.php";
...
//FUNÇÃO QUE VERIFICA LOGIN COM AJAX
$(document).ready(function(){
    var erroLog = $('#erroLog');
    erroLog.hide();
    //Esconde o elemento com id errolog
    $('#formulario_login_prof').submit(function(){ 	//Ao submeter formulário
        var login=$('#lloginn').val();	//Pega valor do campo email
        var senha=$('#ssenhaa').val();
            //Pega valor do campo senha
        $.ajax({			//Função AJAX
            ...
            success: function (result){			//Sucesso no AJAX
                    if(result==1){						
                            location.href="recursos.php";	//Redireciona
                    }else{
                        if(login=='' || senha==''){
                            erroLog.hide('slow');
                        }else{
    ...
    $("#formulario_login_prof").validate({//valide o form indicado por idDoForm
...
//FUNÇÃO PARA DAR UM EFEITO FADEIN NO INDEX DO SGMD
$(document).ready(function(){
    var erroLog = $('#erroLog');
    //Esconde o elemento com id errolog
    $('#formulario_login_aluno').submit(function(){
        //Ao submeter formulário
        var cpf=$("input[name=cpff]").val();	//Pega valor do campo email
        var nascimento=$("input[name=nascimentoo]").val();

            //Pega valor do campo senha
        $.ajax({			//Função AJAX
            url:"login_alunos.php",			//Arquivo php
            type:"post",				//Método de envio
            data: "cpff="+cpf+"&nascimentoo="+nascimento,	//Dados
            success: function (result){			//Sucesso no AJAX
                if(result==1){						
    ...
    $("#formulario_login_aluno").validate({//valide o form indicado por idDoForm
    rules: {//aqui descreva as regras
        cpff: {
            required : true, //regra que indica que email � obrigat�rio
        },
